...
def resolve_output_dir(
    ...
    *,
    ...
    if output_dir:
        p = Path(output_dir)
        if p.is_dir() and (p / "reports" / "findings.json").is_file():
            return p
    with _LOCK:
    ...
    if plugin_slug:
        ...
        report = nested / "reports" / "findings.json"
        if report.is_file() and _read_report_scan_id(report) == scan_id:
            return nested
        legacy = settings.output_root / plugin_slug
        report = legacy / "reports" / "findings.json"
        if _read_report_scan_id(report) == scan_id:
            return legacy
    ...
    if root.is_dir():
        for slug_dir in sorted(root.iterdir(), key=lambda p: p.stat().st_mtime, reverse=True):
            ...
            if _read_report_scan_id(report) == scan_id:
                return nested
            report = slug_dir / "reports" / "findings.json"
            if _read_report_scan_id(report) == scan_id:
                return slug_dir
            if slug_dir.name == scan_id:
                report = slug_dir / "reports" / "findings.json"
                if report.is_file():
                    return slug_dir
    return None
