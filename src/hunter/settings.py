...
class HunterSettings(BaseSettings):
    ...
    def load(cls, yaml_path: Path | None = None) -> HunterSettings:
        ...
        if "parse" in defaults:
            flat["parse_per_file_timeout_seconds"] = float(
                defaults["parse"].get("per_file_timeout_seconds", 120)
            )
            flat["lift_version"] = str(defaults["parse"].get("lift_version", "1"))
        ...
        if "analysis" in defaults:
            rp = defaults["analysis"].get("rule_pack_path") or ""
            flat["rule_pack_path"] = Path(rp) if rp else None
            flat["rule_timeout_seconds"] = float(defaults["analysis"].get("rule_timeout_seconds", 60))
