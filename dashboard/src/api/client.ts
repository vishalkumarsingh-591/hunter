...
export const api = {
  ...
  reportPdfUrl: (id: string, params: Record<string, string> = {}) => {
    const q = new URLSearchParams(params).toString();
    return `${API_BASE}/api/scans/${id}/report.pdf${q ? `?${q}` : ""}`;
  },
  downloadReportPdf: async (id: string, params: Record<string, string> = {}) => {
    const url = api.reportPdfUrl(id, params);
    const res = await fetch(url, { credentials: "same-origin" });
    if (!res.ok) throw new Error(await parseError(res));
    const blob = await res.blob();
    const cd = res.headers.get("Content-Disposition") || "";
    const match = cd.match(/filename\*?=(?:UTF-8'')?["']?([^"';]+)["']?/i);
    const filename = match ? decodeURIComponent(match[1]) : `hunter-report-${id.slice(0, 8)}.pdf`;
